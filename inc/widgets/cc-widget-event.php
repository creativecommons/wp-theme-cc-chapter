...
class WP_Widget_event extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array('classname' => 'widget-event', 'description' => 'List of upcoming events');
        $control_ops = array();
        parent::__construct('widget-event', 'CC Event list', $widget_ops, $control_ops);
    }
    function get_last_events($size) {
        $args = array(
                'post_type' => 'cc_chevent',
                'posts_per_page' => $size,
                'post_status' => 'publish'
            );
        if ( is_single() ) {
    ...
    function widget($args, $instance) {
        ...
        $link_text = ( !empty( $instance['link_text'] ) ) ? $instance['link_text'] : 'See all events';
        $news = $this->get_last_events($size);
        if ( !empty( $news ) ) {
            echo '<div class="widget event-list">';
                if ($show_title) {
                ...
                echo '<div class="widget-content">';
                    echo '<div class="grid-container '.$grid.'">';
                        foreach ($news as $item) {
                            echo '<div class="cell">';
                                echo render::widget_event($item);
                            echo '</div>';
                        }
                    echo '</div>';
                    if ( !empty( $instance['is_link'] ) ) {
                        $link = get_post_type_archive_link( 'cc_chevent' );
...
function cc_event_widget_init()
{
    register_widget( 'WP_Widget_event');
}

add_action('widgets_init', 'cc_event_widget_init');