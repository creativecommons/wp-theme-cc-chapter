...
<section class="featured <?php print $cc_subscribe_theme; ?> num-cols-<?php echo $cc_subscribe_numposts;?>">
	<header>
		<?php if($cc_subscribe_title) { ?><h2><?php print $cc_subscribe_title; ?></h2><?php } ?>
		<?php if($cc_subscribe_description){ ?><div class="widget-description"><?php print $cc_subscribe_description; ?></div><div style="clear:both;"></div><?php } ?>
	</header>
		<?php
		// The hero feature
		...
		<article class="hero">
			<div class="item">
				...
				<div class="teaser">
					...
					<div class="excerpt" <?php if($cc_subscribe_hero_content_display == 'hide'){ ?> style="display:none;" <?php } ?>><?php 
						if($cc_subscribe_hero_content_display == 'excerpt'){ print the_excerpt(); } 
						else { print the_content(); } ?>
					</div>
					<?php if($cc_subscribe_post_read_more) { ?><div class="more"><a href="<?php print $url; ?>"><?php print $cc_subscribe_post_read_more; ?></a></div><?php } ?>
				</div>
			</div>
		</article>
			<?php } ?>
	...
	?>
		<article>
			<div class="item">
				...
				<div class="teaser">
					<h3 class="title"><a href="<?php print $url; ?>"><?php print get_the_title() ?></a></h3>
					<div class="excerpt" <?php if($cc_subscribe_content_display == 'hide'){ ?> style="display:none;" <?php } ?>><?php 
						if($cc_subscribe_content_display == 'excerpt'){ print the_excerpt(); } 
						else { print the_content(); } ?>
					</div>					<?php if($cc_subscribe_post_read_more) { ?><div class="more"><a href="<?php print $url; ?>"><?php print $cc_subscribe_post_read_more; ?></a></div><?php } ?>
				</div>
			</div>
		</article>
		<?php 
		...
		<?php if($cc_subscribe_more) { ?><div class="more"><a href="/?post_type=post"><?php print $cc_subscribe_more; ?><i class="cc-icon-right-dir"></i></a></div><?php } ?>
	</div>
</section>
...
// TITLE
	if( isset( $instance[ 'cc_subscribe_title' ] ) ) {
						$cc_subscribe_title = $instance[ 'cc_subscribe_title' ];} 
	else {				$cc_subscribe_title = __( '', 'wpb_widget_domain' );}
	// DESCRIPTION
	if( isset( $instance[ 'cc_subscribe_description' ] ) ) {
						$cc_subscribe_description = $instance[ 'cc_subscribe_description' ];} 
	else {				$cc_subscribe_description = __( '', 'wpb_widget_domain' ); }
	//MORE ARTICLES
	if( isset( $instance[ 'cc_subscribe_more' ] ) ) {
						$cc_subscribe_more = $instance[ 'cc_subscribe_more' ]; } 
	else {				$cc_subscribe_more = __( '', 'wpb_widget_domain' ); }
	//READ MORE
	if( isset( $instance[ 'cc_subscribe_post_read_more' ])){	
						$cc_subscribe_post_read_more = $instance[ 'cc_subscribe_post_read_more' ];} 
	else {				$cc_subscribe_post_read_more = __( '', 'wpb_widget_domain' ); }
//Repeat for number of posts
	if( isset( $instance[ 'cc_subscribe_numposts' ] ) ) {
						$cc_subscribe_numposts = $instance[ 'cc_subscribe_numposts' ];}
	else {				$cc_subscribe_numposts = __( '3', 'wpb_widget_domain' );}      
//CATEGORY
	if( isset( $instance[ 'cc_subscribe_category' ] ) ) {		
						$cc_subscribe_category = $instance[ 'cc_subscribe_category' ];}
	else {	            $cc_subscribe_category = __( '', 'wpb_widget_domain' );}
//show excerpt
	if( isset( $instance[ 'cc_subscribe_show_excerpt' ] ) ) {
						$cc_subscribe_show_excerpt = $instance[ 'cc_subscribe_show_excerpt' ];}
	else {				$cc_subscribe_show_excerpt = __( '', 'wpb_widget_domain' );}
//Content Display Type
	...
	else {				$cc_subscribe_content_display = __( 'excerpt', 'wpb_widget_domain' );}     
	...
	else {				$cc_subscribe_hero_content_display = __( 'excerpt', 'wpb_widget_domain' );}	  
// Image Ratio
	if( isset( $instance[ 'cc_subscribe_aspect_ratio' ] ) ) {
						$cc_subscribe_aspect_ratio = $instance[ 'cc_subscribe_aspect_ratio' ];}
	else {				$cc_subscribe_aspect_ratio = __( '3_2', 'wpb_widget_domain' );}
// Widget Title
