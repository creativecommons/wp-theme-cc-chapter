...
class CreativeCommons_Widget_Template extends WP_Widget {
   ...
   */
	function widget( $args, $instance ) {
		...
		$noheader = "no-header";
		if(isset($cc_widget_name_title) || isset($cc_widget_name_description)){
			$noheader = false;
		}
?>
<section class="featured <?php print $cc_widget_name_theme; ?> num-cols-<?php echo $cc_widget_name_numposts;?> <?php echo $noheader;?>">

<?php 
	if(!$noheader){ ?>
	<header>
		<?php if($cc_widget_name_title) { ?><h2><?php print $cc_widget_name_title; ?></h2><?php } ?>
		<?php if($cc_widget_name_description){ ?><div class="widget-description"><?php print $cc_widget_name_description; ?></div><div style="clear:both;"></div><?php } ?>
	</header>
	<?php 
	} 
		// The hero feature
		...
		?>
		<article class="hero <?php print $inner_class; ?>">
			<div class="item">
				...
				<div class="teaser">
					...
					<div class="excerpt" <?php if($cc_widget_name_hero_content_display == 'hide'){ ?> style="display:none;" <?php } ?>><?php 
						if($cc_widget_name_hero_content_display == 'excerpt'){ print the_excerpt(); } 
						else { print the_content(); } ?>
					</div>
					<?php if($cc_widget_name_post_read_more) { ?><div class="more"><a href="<?php print $url; ?>"><?php print $cc_widget_name_post_read_more; ?></a></div><?php } ?>
				</div>
			</div>
		</article>
			<?php } ?>
	...
	?>
		<article>
			<div class="item">
				...
				<div class="teaser">
					<h3 class="title"><a href="<?php print $url; ?>"><?php print get_the_title() ?></a></h3>
					<div class="excerpt" <?php if($cc_widget_name_content_display == 'hide'){ ?> style="display:none;" <?php } ?>><?php 
						if($cc_widget_name_content_display == 'excerpt'){ print the_excerpt(); } 
						else { print the_content(); } ?>
					</div>
					<?php if($cc_widget_name_post_read_more) { ?><div class="more"><a href="<?php print $url; ?>"><?php print $cc_widget_name_post_read_more; ?></a></div><?php } ?>
				</div>
	...
	<?php if($cc_widget_name_more) { ?><div class="more"><a href="/?post_type=post"><?php print $cc_widget_name_more; ?><i class="cc-icon-right-dir"></i></a></div><?php } ?>
</section>
...
// TITLE
	if( isset( $instance[ 'cc_widget_name_title' ] ) ) {
						$cc_widget_name_title = $instance[ 'cc_widget_name_title' ];} 
	else {				$cc_widget_name_title = __( '', 'wpb_widget_domain' );}
	// DESCRIPTION
	if( isset( $instance[ 'cc_widget_name_description' ] ) ) {
						$cc_widget_name_description = $instance[ 'cc_widget_name_description' ];} 
	else {				$cc_widget_name_description = __( '', 'wpb_widget_domain' ); }
	//MORE ARTICLES
	if( isset( $instance[ 'cc_widget_name_more' ] ) ) {
						$cc_widget_name_more = $instance[ 'cc_widget_name_more' ]; } 
	else {				$cc_widget_name_more = __( '', 'wpb_widget_domain' ); }
	//READ MORE
	if( isset( $instance[ 'cc_widget_name_post_read_more' ])){	
						$cc_widget_name_post_read_more = $instance[ 'cc_widget_name_post_read_more' ];} 
	else {				$cc_widget_name_post_read_more = __( '', 'wpb_widget_domain' ); }
...
//Content Display Type
	...
	else {				$cc_widget_name_content_display = __( 'excerpt', 'wpb_widget_domain' );}     
	...
	else {				$cc_widget_name_hero_content_display = __( 'excerpt', 'wpb_widget_domain' );}	  
// Image Ratio
	if( isset( $instance[ 'cc_widget_name_aspect_ratio' ] ) ) {
						$cc_widget_name_aspect_ratio = $instance[ 'cc_widget_name_aspect_ratio' ];}
	else {				$cc_widget_name_aspect_ratio = __( '3_2', 'wpb_widget_domain' );}
// Widget Title
...
<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="hidden" value="<?php echo esc_attr( $cc_widget_name_category ); ?>" />
   ...
   */
	public function update( $new_instance, $old_instance ) {
		...
		$instance['cc_widget_name_title'] = ( ! empty( $new_instance['cc_widget_name_title'] ) ) ? strip_tags( $new_instance['cc_widget_name_title'],"<br>" ) : null;
