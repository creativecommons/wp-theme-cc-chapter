...
class WP_Widget_programs extends WP_Widget {
    /** constructor */
    function __construct() {
        $widget_ops = array('classname' => 'widget-programs', 'description' => 'List program areas');
        $control_ops = array();
        parent::__construct('widget-programs', 'CC Program Areas', $widget_ops, $control_ops);
    }
    function get_last_programs($size) {
        $args = array(
                'post_type' => 'cc_charea',
                'posts_per_page' => $size,
                'post_status' => 'publish'
    ...
    function widget($args, $instance) {
        ...
        $link_text = ( !empty( $instance['link_text'] ) ) ? $instance['link_text'] : 'See all Program Areas';
        $news = $this->get_last_programs($size);
        if ( !empty( $news ) ) {
                ...
                echo '<div class="widget-content">';
                    echo '<div class="grid-container '.$grid.'">';
                        foreach ($news as $item) {
                            echo '<div class="cell">';
                                echo render::widget_program($item);
                            echo '</div>';
                        }
                    echo '</div>';
...
function cc_text_programs_widget_init()
{
    register_widget( 'WP_Widget_programs');
}

add_action('widgets_init', 'cc_text_programs_widget_init');