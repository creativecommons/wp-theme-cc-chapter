...
// TITLE
	if( isset( $instance[ 'cc_programs_title' ] ) ) {
						$cc_programs_title = $instance[ 'cc_programs_title' ];} 
	else {				$cc_programs_title = __( 'Featured</br>Program Areas', 'wpb_widget_domain' );}
	// DESCRIPTION
	if( isset( $instance[ 'cc_programs_description' ] ) ) {
						$cc_programs_description = $instance[ 'cc_programs_description' ];} 
	else {				$cc_programs_description = __( '', 'wpb_widget_domain' ); }
	//MORE ARTICLES
	if( isset( $instance[ 'cc_programs_more' ] ) ) {
						$cc_programs_more = $instance[ 'cc_programs_more' ]; } 
	else {				$cc_programs_more = __( 'See all program areas', 'wpb_widget_domain' ); }
	//READ MORE
	if( isset( $instance[ 'cc_programs_post_read_more' ])){	
						$cc_programs_post_read_more = $instance[ 'cc_programs_post_read_more' ];} 
	else {				$cc_programs_post_read_more = __( '', 'wpb_widget_domain' ); }
//Repeat for number of posts
	if( isset( $instance[ 'cc_programs_numposts' ] ) ) {
						$cc_programs_numposts = $instance[ 'cc_programs_numposts' ];}
	else {				$cc_programs_numposts = __( '4', 'wpb_widget_domain' );}      
//Content Display Type
	if( isset( $instance[ 'cc_programs_content_display' ] ) ) {
						$cc_programs_content_display = $instance[ 'cc_programs_content_display' ];}
	else {				$cc_programs_content_display = __( 'excerpt', 'wpb_widget_domain' );}     

// Widget Title
	if( isset( $instance[ 'title' ] ) ) {
						$title = $instance[ 'title' ];}
	else {				$title = __( 'Programs', 'wpb_widget_domain' );}
