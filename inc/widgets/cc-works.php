...
class CreativeCommons_Works extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		<section class="featured-works widget  num-cols-<?php echo $cc_work_numposts; ?> <?php echo $noheader; ?>">
			...
			if ( ! $noheader ) {
				...
				<header>
					...
					if ( $cc_work_title ) {
						?>
						<h2><?php print $cc_work_title; ?></h2><?php } ?>
					<?php
					if ( $cc_work_description ) {
						?>
						<div class="widget-description"><?php print $cc_work_description; ?></div><div style="clear:both;"></div><?php } ?>
				</header>
				<?php
			}
			// The hero feature
			$the_query = cc_widgets::get_featured_posts_by_taxonomy_query( 'work', 'hero', 1 );
				...
				<article class="hero">
					<div class="item">
						...
						<div class="teaser">
                            ...
                            <?php $add_style = ($cc_work_hero_content_display == 'hide') ? 'style="display:none;"': ''; ?>
							<div class="excerpt" <?php echo $add_style; ?> >
                            <?php
                            if ( $cc_work_hero_content_display == 'excerpt' ) {
                                print the_excerpt(); 
                            } else {
                                print the_content(); 
                            }
							...
							<?php
							if ( $cc_work_post_read_more ) {
								?>
								<div class="more"><a href="<?php print $url; ?>"><?php print $cc_work_post_read_more; ?></a></div><?php } ?>
						</div>
					</div>
				</article>
			<?php } ?>
			...
			// The other features
				$the_query = cc_widgets::get_featured_posts_by_taxonomy_query( 'work', 'highlight', $cc_work_numposts );
			while ( $the_query->have_posts() ) {
				$the_query->the_post();
				$hero_post_id = get_the_ID();
				$url          = get_permalink();
				...
				?>
					<article>
						<div class="item">
							...
							<div class="teaser">
                                <h3 class="title"><a href="<?php print $url; ?>"><?php print get_the_title(); ?></a></h3>
                                <?php $add_style = ($cc_work_content_display == 'hide') ? 'style="display:none;"': ''; ?>
								<div class="excerpt" <?php echo $add_style; ?> >
                                <?php
                                    if ( $cc_work_content_display == 'excerpt' ) {
                                        print the_excerpt(); 
                                    } else {
                                        print the_content(); 
                                    }
		...
		if ( $cc_work_more ) {
			?>
			<div class="more"><a href="/?post_type=post"><?php print $cc_work_more; ?><i class="cc-icon-right-dir"></i></a></div><?php } ?>
	</section>
	...
	public function form( $instance ) {
		$cc_work_title                = ( ! empty( $instance['cc_work_title'] ) ) ? $instance['cc_work_title'] : __( 'Featured<br>Works', 'wpb_widget_domain' );
		$cc_work_description          = ( ! empty( $instance['cc_work_description'] ) ) ? $instance['cc_work_description'] : __( '<strong>1.1 billion works and counting.</strong> Explore these featured Creative Commons Licensed resources below, or you can <a href="/share-your-work/">share your work</a>,  and help light upthe global commons!', 'wpb_widget_domain' );
		$cc_work_more                 = ( ! empty( $instance['cc_work_more'] ) ) ? $instance['cc_work_more'] : __( '', 'wpb_widget_domain' );
		$cc_work_post_read_more       = ( ! empty( $instance['cc_work_post_read_more'] ) ) ? $instance['cc_work_post_read_more'] : __( '', 'wpb_widget_domain' );
		$cc_work_numposts             = ( ! empty( $instance['cc_work_numposts'] ) ) ? $instance['cc_work_numposts'] : __( '4', 'wpb_widget_domain' );
		$cc_work_show_excerpt         = ( ! empty( $instance['cc_work_show_excerpt'] ) ) ? $instance['cc_work_show_excerpt'] : __( '', 'wpb_widget_domain' );
		$cc_work_content_display      = ( ! empty( $instance['cc_work_content_display'] ) ) ? $instance['cc_work_content_display'] : __( 'excerpt', 'wpb_widget_domain' );
		$cc_work_hero_content_display = ( ! empty( $instance['cc_work_hero_content_display'] ) ) ? $instance['cc_work_hero_content_display'] : __( 'excerpt', 'wpb_widget_domain' );
		$title                        = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Works', 'wpb_widget_domain' );
